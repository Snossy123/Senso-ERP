...
class SalesInvoiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->boolean('confirm_now')) {
            return $this->redirectAfterConfirm($request, $invoice);
        }
    ...
    protected function redirectAfterConfirm(Request $request, SalesInvoice $invoice)
    {
        $this->authorizePermission('sales_invoices.confirm');

        $installmentPlan = null;
        if (($request->input('payment_term') ?? $invoice->payment_term) === 'installment') {
            $installmentPlan = [
                'down_payment' => $request->input('down_payment', 0),
                'installment_count' => $request->input('installment_count', 1),
                'interval_days' => $request->input('interval_days', 30),
                'first_due_date' => $request->input('first_due_date', now()->addMonth()->toDateString()),
            ];
        }
